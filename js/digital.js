...
if (document.location.href.includes("project-pets")) {
  ...
  /*nếu địa chỉ URL của trang web bao gồm chuỗi "project-cv", thay đổi nội dung của thẻ h1 thành "Project charter - CV WEB" và sau đó sử dụng phương thức forEach() để thiết lập thuộc tính display thành "flex". */
} else if (document.location.href.includes("project-news")) {
  title.innerHTML = "Project Charter - NEWS WEB";
  all.forEach((item) => {
    item.style.display = "flex";
  });
  /*nếu địa chỉ URL của trang web bao gồm chuỗi "project-news", thay đổi nội dung của thẻ h1 thành "Project charter - NEWS WEB" và sau đó sử dụng phương thức forEach() để thiết lập thuộc tính display thành "flex". */
}
